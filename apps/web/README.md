...
Next.js App Router + TypeScript + pnpm + shadcn/ui (Radix) + Tailwind CSS v4. Principal 신원 확인·Delegation 발급 콘솔입니다. 키 보관, JWT 발급, DID 문서, Treasury 송금, Daytona 샌드박스 실행은 모두 `@kya/core`를 호출하며, 이 앱은 HTTP 라우트·화면 상태·검증 규칙만 갖습니다. `apps/verifier`와는 `VERIFIER_URL`로만 연결됩니다.

## 실행

```sh
pnpm install                       # 저장소 루트에서. 워크스페이스 전체를 설치한다
cp apps/web/.env.example apps/web/.env.local
pnpm web                           # = pnpm --filter @kya/web dev
```
...
`.env.local`에서 `WEB_DEMO_MODE=false`와 Daytona·Anthropic·Treasury 환경 변수를 설정합니다. `PUBLIC_HOST`는 러너가 접근할 수 있는 터널 호스트, `RUNNER_DIR`은 팀원 러너의 절대 경로입니다. 실행 명령은 `RUNNER_COMMAND`이며 기본값은 core와 같은 `npm install --omit=dev && node index.js`입니다.

core의 샌드박스 런처가 러너 디렉터리를 `/home/daytona/runner`에 업로드하고 백그라운드로 실행합니다. `node_modules`, `.git`, `dist`, `.next`는 업로드하지 않습니다. 샌드박스 생성 후 알 수 있는 `SANDBOX_ID`는 실행 시 주입하며, 나머지 환경 변수는 계약 접점 4를 따릅니다. 실제 위임 발급은 Treasury의 USDC(`FUNDING_AMOUNT`, 기본 20 USDC) 전송 영수증까지 확인해야 완료됩니다. 대기 중인 트랜잭션 해시를 저장하므로 재시도 시 동일 해시를 확인합니다. RPC가 트랜잭션을 수신한 뒤 응답을 유실하는 경우까지 보장하는 분산 송금 멱등성은 구현하지 않았습니다.

서버 시작 시 `data/principal-key.json`에 Ed25519 키를 core 형식(`kid`, `publicJwk`, `privateJwk`)으로 생성하고 재사용합니다. 이전 웹 버전이 만든 키 파일은 형식이 달라 `data/`를 지우고 다시 시작해야 합니다. `data/state.json`에 화면 상태를 저장합니다. 이 파일들은 Git에서 제외됩니다. 로컬 단일 서버 프로세스와 지속 디스크를 전제로 하며 서버리스 배포용 저장소는 아닙니다. 데모를 초기화하려면 서버 종료 후 `data/state.json`을 삭제합니다. 키 파일은 유지할 수 있습니다.
...
Verifier 요청은 서버의 `/api/verifier/decisions`, `/api/verifier/ledger/:jti`를 통해 전달하므로 브라우저 CORS 설정 없이 연결됩니다. Verifier 검증·정산 자체는 `apps/verifier`의 책임입니다.
...
# 저장소 루트에서: 웹 + Verifier 를 함께 띄워 데모 4장면
pnpm e2e
```
